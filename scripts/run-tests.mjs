...
//   integration  several real modules wired together. Still hermetic — never the real codex CLI.
//   e2e          the real codex CLI driven through the real MCP server over stdio. Opt-in (live).
//   judge        live Codex CLI output evaluated by a second LLM judge. Opt-in (live).
...
// Mute routine [CMCPT] logging for the hermetic categories so the reporter
...
// a single shared on-disk dir (os.tmpdir()/codex-mcp-chunks); files that touch
// it (chunkCache, changeMode-pipeline) would otherwise race across parallel
// worker processes. Serial e2e also avoids hitting the codex backend in parallel.
