...
// Server + protocol + the tools that do NOT need the codex CLI. These run
// anywhere the project is built, with no codex install or network.
...
describe("MCP Protocol E2E: Server Lifecycle & System Tools", () => {
  test("lists every registered tool with a valid input schema", async (t) => {
    ...
    for (const expected of ["ask-codex", "brainstorm", "fetch-chunk", "ping", "Help"]) {
      assert.ok(names.includes(expected), `tools/list is missing "${expected}" (got: ${names.join(", ")})`);
    }
    const ask = tools.find((t) => t.name === "ask-codex");
    assert.ok(ask);
  ...
  test("lists prompts derived from the registry", async (t) => {
    ...
    assert.ok(prompts.map((p) => p.name).includes("ask-codex"));
  });
