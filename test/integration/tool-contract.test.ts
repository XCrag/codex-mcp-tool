...
// invoking the Codex CLI: argument validation, and the guard/error branches
// inside the tools. (The happy path that actually calls Codex is covered by
...
describe("MCP Subsystem Integration: Tool Input Validation Contracts", () => {
  test("executeTool surfaces zod validation as a friendly error", async () => {
    // ask-codex requires a non-empty prompt; the error names the offending field.
    await assert.rejects(() => executeTool("ask-codex", {}), /Invalid arguments for ask-codex.*prompt/s);
  });
  ...
  test("ask-codex rejects a malformed chunkCacheKey before calling Codex", async () => {
    const out = await executeTool("ask-codex", {
      prompt: "x",
      changeMode: true,
      chunkIndex: 1,
  ...
  test("ask-codex changeMode continuation with a missing cache reports a cache miss (no spawn)", async () => {
    // Well-formed key, but nothing cached -> the continuation path returns the
    // cache-miss message rather than shelling out to Codex.
    const out = await executeTool("ask-codex", {
      prompt: "x",
      changeMode: true,
      chunkIndex: 1,
