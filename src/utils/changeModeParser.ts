...
export function parseChangeModeOutput(codexResponse: string): ChangeModeEdit[] {
  const edits: ChangeModeEdit[] = [];
  const markdownPattern = /\*\*FILE:\s*(.+?):(\d+)\*\*\s*\n```\s*\nOLD:\s*\n([\s\S]*?)\nNEW:\s*\n([\s\S]*?)\n```/g;

  let match;
  while ((match = markdownPattern.exec(codexResponse)) !== null) {
  ...
  if (edits.length === 0) {
    ...
    while ((match = editPattern.exec(codexResponse)) !== null) {
      const [
        _fullMatch,
        oldFilename,
        oldStartLine,
