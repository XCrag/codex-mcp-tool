...
import { startServer, callCodex, callTool, textOf, CODEX_SKIP, type ServerHandle } from "../e2e/harness.js";
...
export const RUBRICS = {
  // Ask Codex General ( model selection [-m], sandbox policy, and changeMode:boolean for providing edits ).
  askCodexGeneral: `
...
3. The output must include a "Codex Execution Report" blockquote at the very bottom.
4. The execution report must list: Model, Latency, Tokens (est), and Cost (est).
`,

  // Ask Codex Change Mode ( model selection [-m], sandbox policy, and changeMode:boolean for providing edits ).
  askCodexChangeMode: `
...
4. The output must include the "Codex Execution Report" blockquote at the bottom.
`,
...
5. The output must include the "Codex Execution Report" blockquote at the bottom.
`,
};
...
export async function runJudge(
  ...
  if (config.deepseekApiKey) {
    ...
    extraHeaders = {
      "HTTP-Referer": "https://github.com/codex-mcp-tool",
      "X-Title": "Codex MCP Tool E2E Judge",
    };
  } else {
    throw new Error(
...
// Skip criteria: needs the real codex CLI *and* a judge API key
const skip = CODEX_SKIP || (!hasJudgeKey ? "No DeepSeek/OpenRouter API key configured in test/.env" : false);
const options = { skip, timeout: 240_000 } as const;

let server: ServerHandle;
const TEST_ENV = { CODEX_MCP_TEST_TOOLS: "1" };
...
function buildExecutionReport(
  ...
  // Rough est only — the rubric checks that a cost line is present, not its accuracy.
  const cost = (promptTokens * 1.25 + responseTokens * 10.0) / 1000000;
...
describe("MCP Tool Semantic Evaluations (LLM-as-a-Judge)", () => {
  test("ask-codex general response meets Q&A rubric", options, async (t) => {
    ...
    const evaluation = await runJudge(prompt, textWithReport, RUBRICS.askCodexGeneral);
  ...
  test("ask-codex changeMode output meets structured edits rubric", options, async (t) => {
    // Read the version from package.json dynamically, so it always matches the active project environment
    let versionPrompt = "from 0.1.0 to 0.1.1";
    ...
    try {
      const startTime = Date.now();
      const { isError, text } = await callCodex(t, testServer, {
        name: "ask-codex",
      ...
      const evaluation = await runJudge(prompt, textWithReport, RUBRICS.askCodexChangeMode);
  ...
  test("ask-codex handles client-side timeout gracefully", options, async (t) => {
