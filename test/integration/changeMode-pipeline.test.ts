...
// This wires the full changeMode path that the ask-codex tool drives for large
// edits: a Codex-style response string -> parse -> validate -> chunk -> cache,
// then the fetch-chunk tool retrieving subsequent chunks. No CLI is involved —
// the "Codex output" is a fixture string, so the test is hermetic.
import { processChangeModeOutput } from "../../src/utils/codexExecutor.js";
...
describe("MCP Subsystem Integration: changeMode Pipeline", () => {
  ...
  test("a response with no OLD/NEW edits yields a clear message", async () => {
    const out = await processChangeModeOutput("Codex replied with prose and no edits.", undefined, undefined, "prompt-none");
