...
// entire product: protocol -> registry -> tool -> backend -> spawned codex CLI.
//
...
/** True when the real codex CLI is installed and resolvable on PATH. */
export function hasCodex(): boolean {
  const isWindows = process.platform === "win32";
  const command = resolveCommandForExecution(CLI.COMMANDS.CODEX);
...
/** Skip reason for codex-dependent tests, or false when codex is available. */
export const CODEX_SKIP: string | false = hasCodex()
  ? false
  : "codex CLI not on PATH — run `npm i -g @openai/codex` and authenticate (`codex login`)";
...
export async function startServer(
  ...
  // Pass the parent environment through (PATH so codex resolves, HOME so the
  // codex auth/config is found), plus any per-test overrides.
  ...
  const client = new Client({ name: "cmcpt-e2e", version: "0.0.0" }, { capabilities: {} });
 ...
 * `codex --help`) so they aren't retried needlessly.
 */
