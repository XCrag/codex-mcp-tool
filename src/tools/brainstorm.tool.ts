...
const brainstormArgsSchema = z.object({
  ...
  model: z.string().optional().describe("Optional model override. Only pass when the user explicitly requests a specific model."),
...
export const brainstormTool: UnifiedTool = {
  ...
  execute: async (args, onProgress) => {
    ...
    if (!result.success) {
      return `❌ Codex brainstorm failed.${result.errorText}`;
    }
