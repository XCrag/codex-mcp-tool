 ...
 * This allows you to run 'askCodexChangeMode' via judge while testing codex-mcp-tool.
 */
...
export function loadConfig(envPath: string = path.join(path.dirname(fileURLToPath(import.meta.url)), ".env")): TestConfig {
  const config: TestConfig = {
    judgeCodexModel: process.env.JUDGE_CODEX_MODEL || "gpt-5-codex",
  ...
  if (fs.existsSync(envPath)) {
    try {
      ...
      for (const line of lines) {
        ...
        } else if (key === "JUDGE_CODEX_MODEL" && val && !process.env.JUDGE_CODEX_MODEL) {
          config.judgeCodexModel = val;
        }
      }
    } catch (e) {
